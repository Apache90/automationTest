...
export class ValidadorModal {
    ...
    async esperarModalExito() {
        ...
        await expect(this.modal).toContainText('Nuevo Validador agregado correctamente');
    }
    ...
    async confirmarEliminacionValidador() {
        // Espera el modal de confirmación de eliminación
        await expect(this.modal).toContainText('¿Estás seguro que deseas eliminar este Validador?');
        const confirmar = this.page.locator('.dialog-button', { hasText: 'Confirmar' });
        await confirmar.click();
    }

    async esperarModalEliminacionExitosa() {
        await expect(this.modal).toContainText('Validador quitado correctamente.');
