...
export class VendedorModal {
    ...
    async esperarModalExito() {
        ...
        await expect(this.modal).toContainText(/Nuevo Vendedor agregado correctamente\./i);
    }
    ...
    async confirmarEliminacionVendedor() {
        ...
        await expect(this.modal).toBeVisible({ timeout: 20000 });
        await expect(this.modal).toContainText('¿Estás seguro que deseas eliminar este Vendedor?');
    ...
    async esperarModalEliminacionExitosa() {
        await expect(this.modal).toBeVisible({ timeout: 20000 });
        await expect(this.modal).toContainText('Vendedor quitado correctamente.');
