...
test.describe('Flujo de Encargado', () => {
  ...
  test('El Encargado puede iniciar sesión y ver su bienvenida', async ({ page }) => {
    ...
    allure.description('Verifica que un encargado pueda iniciar sesión correctamente y ver su mensaje de bienvenida personalizado');
  ...
  test.describe('Gestión de Roles', () => {
    ...
    test.describe('Gestión de Supervisores', () => {
      ...
      test('Muestra mensaje si el supervisor ya posee el rol indicado', async ({ page }) => {
        allure.description('Verifica que se muestre un mensaje de error cuando se intenta agregar un supervisor con rol ya existente');
        allure.severity('normal');
      ...
      test('Puede eliminar un supervisor existente y ver confirmación', async ({ page }) => {
        allure.description('Verifica que un encargado pueda eliminar un supervisor existente y recibir confirmación de la operación');
        allure.severity('critical');
  ...
  test.describe('Gestión de Cupones', () => {
    ...
    test.describe("DNI's", () => {
      ...
      test('Muestra error cuando no se ingresa el nombre del cupón', async ({ page }) => {
        allure.description('Verifica que se muestre un mensaje de error cuando se intenta crear un cupón DNI sin nombre');
      ...
      test('Puede crear un nuevo cupón DNI y ver confirmación', async ({ page }) => {
        allure.description('Verifica que un encargado pueda crear un nuevo cupón DNI gratuito y recibir confirmación de éxito');
        allure.severity('critical');
      ...
      test('Puede editar un cupón DNI y ver confirmación', async ({ page }) => {
        allure.description('Verifica que un encargado pueda editar un cupón DNI existente y recibir confirmación de la modificación');
        allure.severity('critical');
      ...
      test('Puede eliminar un cupón DNI y ver confirmación', async ({ page }) => {
        allure.description('Verifica que un encargado pueda eliminar un cupón DNI existente y recibir confirmación de la eliminación');
        allure.severity('critical');
        ...
        const cupon = page.locator('.grid-container .item-header', { hasText: 'DNI FREE 2' }).locator('xpath=ancestor::div[contains(@class, "grid-container")]');
        await expect(cupon).toBeVisible({ timeout: 5000 });
        ...
        await expect(modalConfirmacion).toContainText('¿Estás seguro que deseas eliminar esta lista?');
        ...
        await expect(modalExito.locator('.dialog-text')).toContainText('Cupón eliminado con éxito');
        const botonOK = modalExito.locator('span.dialog-button', { hasText: 'OK' });
        await botonOK.click();

        // Verificar que el cupón ha sido eliminado
        const cuponEliminado = page.locator('.grid-container .item-header', { hasText: 'DNI FREE 2' });
        await expect(cuponEliminado).toHaveCount(0, { timeout: 5000 });

        // Verificar el mensaje "No hay cupones disponibles"
        const mensajeSinCupones = page.locator('p', { hasText: 'No hay cupones disponibles.' });
        await expect(mensajeSinCupones).toBeVisible();
        await expect(mensajeSinCupones).toHaveText('No hay cupones disponibles.');
      });
    });
